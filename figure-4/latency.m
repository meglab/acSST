function [res,cfgNew] = latency(cfg,avgs,sign,meanBase)
...
for subi=1:nSubs %cycle through subjects
    ...
    if cfg.get.counterPeak
        ...
        %tmp=abs((meanBase(subi)-peak.amp)*(1-cfg.percAmp));
        tmp=abs((50-peak.amp)*(1-cfg.percAmp));
        comp.baseline(subi)=50+tmp;
    end
    ...
    for i=1:length(checkFields)
        if isfield(cfg,checkFields{i}) && isnumeric(cfg.(checkFields{i}))
            if isempty(regexp(checkFields{i},'Width','once'))
                ...
                if cfg.warnings && any(abs(cfg.(checkFields{i})-cfg.times(samplesOut))>0.1)
